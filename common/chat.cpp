...
static std::string llama_model_meta_val_str_cpp(const struct llama_model * model, const char * key) {
    if (model == nullptr) {
        return "";
    }
    ...
    value.resize(len + 1);
    int32_t check = llama_model_meta_val_str(model, key, &value[0], value.size());
    if (check < 0) {
        return "";
    }
    value.resize(check);
...
static std::string common_token_to_piece(const struct llama_model * model, llama_token token, bool special) {
    std::string piece;
    piece.resize(16);
...
common_chat_templates_ptr common_chat_templates_init(const struct llama_model * model,
    ...
    if (chat_template_override.empty()) {
        ...
        default_template_src = llama_model_meta_val_str_cpp(model, "tokenizer.chat_template");
        if (!default_template_src.empty()) {
            has_explicit_template = true;
        }
        template_tool_use_src = llama_model_meta_val_str_cpp(model, "tokenizer.chat_template.tool_use");
    ...
    if (model) {
        const auto   get_token = [&](llama_token token, const char * name, const char * jinja_variable_name) {
            ...
            return common_token_to_piece(model, token, true);
        };
        token_bos = get_token(llama_token_bos(model), "BOS", "bos_token");
        token_eos = get_token(llama_token_eos(model), "EOS", "eos_token");
...
static common_chat_params common_chat_templates_apply_legacy(const struct common_chat_templates *        tmpls,
    ...
    int32_t      res = llama_chat_apply_template(nullptr, src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt,
                                                 buf.data(), buf.size());
    ...
    if ((size_t) res > buf.size()) {
        buf.resize(res);
        res = llama_chat_apply_template(nullptr, src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt, buf.data(),
                                        buf.size());
    }
